      ...
      <form onSubmit={handleSaveConcert} className="space-y-8 bg-gray-900 p-8 rounded-2xl border border-gray-800">
        ...
        <div className="flex items-center gap-4 bg-gray-800 p-4 rounded-xl border border-gray-700">
          ...
          <button type="button" onClick={() => setFormData({...formData, isPublished: !formData.isPublished})}
            className={`px-4 py-2 rounded font-bold transition ${formData.isPublished ? 'bg-green-600 text-white' : 'bg-yellow-600 text-white'}`}>
        ...
        {!id && (
          <div className="border-t border-gray-700 pt-8">
            ...
            <div className="flex flex-wrap gap-4 items-end bg-gray-800 p-4 rounded-xl">
              <div><label className="block text-xs text-gray-400 mb-1">โซน (เช่น VIP)</label><input type="text" value={seatInput.tier} onChange={e => setSeatInput({...seatInput, tier: e.target.value})} className="w-24 p-2 bg-gray-700 rounded text-white" /></div>
